...
//
// src-cordis/plugins/provider/lib/taskmap.ts — 会话→任务映射读取（provider 子集）
//
// App 主进程（src/lib/task-map.ts）在 session.prompt 提交前把 { taskId, dshSessionId,
...
// DSHANA_HOME（main.ts 设置）。子进程与 App 共用该目录（runtime writeRoots 含 dataDir）。
// 本模块是 src/lib/task-map.ts 的只读子集（cordis 构建树不 import src/lib）。
// 零依赖纯函数。
...
/** dataDir（process.env.DSHANA_HOME，main.ts 设置）→ 任务映射目录。 */
 ...
 *  不能伪装成“用户自建会话”（三态判定见 lib/identity.ts）。
 */
