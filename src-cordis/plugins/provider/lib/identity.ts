...
// ④ 是关键：“文件读不到”不能降级成 App 身份——那等于把“状态丢了”伪装成“用户会话”，
// 对一条还活着的任务就是丢掉绑定、结果不回投，静默失败。
//
// 判定依据：<dataDir>/dshana/taskmaps/<sessionId>.json（src/lib/task-map.ts 写的单一事实源）。
...
import { readTaskMap } from "./taskmap.ts";

/**
 * @param {string|null} dataDir 受管 runtime 进程的 DSHANA_HOME（main.ts 注入）
