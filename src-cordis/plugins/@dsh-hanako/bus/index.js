...
//
// @dsh-hanako/bus — dsh 侧进程间消息总线服务端（dshana.bus）
//
...
export const name = '@dsh-hanako/bus'
...
export function apply(ctx, config) {
  try {
    ...
    ctx.inject(['webServer'], (httpCtx) => {
      httpCtx.effect(() => {
        let bridgeLog = (msg) => {
          ...
          try {
            ctx.logger?.info?.('[@dsh-hanako/bus] ' + msg)
          } catch {
            /* 日志失败不阻断 */
          }
        }
    ...
    try {
      ctx.logger?.warn?.('[@dsh-hanako/bus] 插件停用：' + ((e && e.message) || e))
    } catch {
      /* 日志失败不阻断 */
    }
  }
