...
//
// @dsh-hanako/bridge — 免鉴权 connection 等价服务 + /api HTTP 载体
//
      ...
      webCtx.effect(() => {
        ...
        if (webCtx.webServer && typeof webCtx.webServer.register === 'function') {
          try {
            ...
            ctx.logger?.warn?.('[@dsh-hanako/bridge] /api 路由注册失败：' + ((e && e.message) || e))
          }
        } else {
          ctx.logger?.warn?.('[@dsh-hanako/bridge] webServer.register 不可用（宿主版本过旧），/api 载体不可用')
        }
        return () => {
