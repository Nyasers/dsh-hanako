...
function hostBase() {
  ...
  const web = g?.web;
  if (!web?.ready || !web.port) throw new Error("dsh web host 未就绪（有审批挂起说明任务正在运行）");
...
export const description =
  "应答 dsh 任务挂起的权限审批（approval/requested）：dsh_run 任务运行中若 dsh agent 请求" +
  "超出沙箱范围的权限（如提权执行命令），任务会挂起等待应答，本工具经宿主 deferred 通道" +
  "通知 Agent。调用本工具选择：allowed-once=放行该次请求（agent 继续执行）/ rejected=拒绝" +
  "（agent 收到拒绝后通常改用其他方式）。opId 与 approvalId 来自审批通知；应答前可先评估" +
  "审批的 toolName 与 reason 决定放行还是拒绝。无人应答时审批也可在 dsh Web UI 人工处理。";
...
export const parameters = {
  ...
  properties: {
    ...
    approvalId: {
      type: "string",
      description: "审批 id（审批通知里带；同一任务可能挂起多个审批，逐个应答）",
    },
    outcome: {
      type: "string",
      enum: ["allowed-once", "rejected"],
      description: "allowed-once=放行单次（默认，安全默认值：仅本次操作）/ rejected=拒绝该请求",
    },
  },
...
export const sessionPermission = {
  ...
  describeSideEffect: () => ({
    kind: "external_api",
    summary: "向 dsh web host 提交审批应答（allowed-once/rejected），放行或拒绝 dsh agent 的越界权限请求",
...
async function doExecute(input, ctx) {
  ...
  if (!op) throw new Error(`op 不存在或已过期（${opId}）：只可应答本会话近期提交的 dsh 任务审批`);
  const approvals = Array.isArray(op.pendingApprovals) ? op.pendingApprovals : [];
  const ap = approvals.find((a) => a.approvalId === approvalId);
  if (!ap) {
    const known = approvals.map((a) => `${a.approvalId}(${a.status})`).join(", ") || "无";
    throw new Error(`审批 ${approvalId} 不在任务 ${opId} 的待办列表（可能已被应答/超时）。已知: ${known}`);
  }
  if (ap.status !== "pending") {
    throw new Error(`审批 ${approvalId} 已${ap.status === "answered" ? `应答（${ap.outcome}）` : ap.status}，勿重复应答`);
  }
  ...
  if (!j.accepted) {
    throw new Error(`审批应答未接受（${j.reason || "unknown"}）：可能已超时或被其他方处理，任务侧会自行感知`);
  }
  ...
  const reasonLine = ap.reason ? `（理由：${String(ap.reason).slice(0, 300)}）` : "";
  return {
    content: [
      {
        type: "text",
...
export async function execute(input, ctx) {
  try {
    ...
    ctx.log?.error?.("[dsh-hanako] dsh_approve failed:", e?.stack || e?.message || String(e));
    throw e;
  }
}
