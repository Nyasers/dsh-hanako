...
//   postbump  → syncver && commit/tag（bump 改版本后同步一次，随 bump 提交）
// 语义：bump 是改版本号唯一入口；同步跟随 bump（postbump）；prepack 兜底任何直 pack
// 场景（绕过 bump 的手改主版本也在出包前被同步/校验）。幂等：已一致零写入。
// 用法：
...
// bump/postbump/syncver 复用——通用构件放根级，领域特有随源码）
...
function main() {
  ...
  if (checkOnly) {
    if (stale.length) {
      ...
      console.error("[syncver] 跑 node scripts/syncver.mjs 同步后提交（或走 pnpm run bump）");
      process.exit(1);
    }
