...
// 背景：pack 曾用 `tar -a -cf xxx.zip`，Windows bsdtar 认 zip，但 GNU tar（Linux）
// 不认 .zip 后缀会静默产出 tar 伪 zip（无 PK 头/无 EOCD），安装端报
// "end of central directory record signature not found"。pack 已改 archiver（纯 Node），
// 本脚本在 CI verify 阶段兜底：魔数 + EOCD + sha256 三重校验，缺一即 fail（删 release）。
// 用法：
...
if (
  ...
  console.error(
    `::error::NOT a zip: ${zipPath} (missing local file header magic PK\\x03\\x04)`,
  );
  process.exit(1);
}
...
if (eocdAt < 0) {
  console.error(
    `::error::NOT a zip: ${zipPath} (missing end of central directory record)`,
  );
  process.exit(1);
}
...
console.log(
  `zip OK: ${zipPath} (${buf.length} bytes, EOCD at -${tail.length - eocdAt})`,
);
