...
//
// scripts/postversion.mjs — 发版收尾：派生版本同步 + git commit/tag（npm postversion 阶段）
// 由 pnpm run version 钩子链的 postversion 触发（package.json scripts.postversion），
// 在 version 阶段（scripts/version.mjs 已 bump 主 package.json）之后执行：
...
//      annotated tag v…
// CHANGELOG 由发版人自行维护（bump 前写好并提交）；打包验证已由 preversion 钩子先行
// （pnpm run pack）完成——本脚本只做版本号文件的机械同步与提交收尾。
// 注意：依赖 pnpm 钩子链（version 后无自动 commit）；勿用 npm version 混跑（npm 会在
// version 后自行 commit/tag，不含本脚本同步的派生文件——见 version.mjs 头注释）。
// 手动补跑：node scripts/postversion.mjs（读主 package.json version 对齐派生 + 提交）。
...
const run = (cmd, desc) => {
  console.log("[postversion] " + desc + "...");
  try {
    execSync(cmd, { stdio: "inherit", cwd: ROOT, shell: true });
  } catch (e) {
    console.error("[postversion] " + desc + " 失败: " + e.message);
    process.exit(1);
  }
};

// cordis 包清单（与 version.mjs 同构；同步对象 = src-cordis 顶层 + plugins/*）
...
function main() {
  ...
  if (typeof version !== "string" || !version) {
    console.error("[postversion] package.json version 缺失");
    process.exit(1);
  }
  ...
  if (manifest.version !== version) {
    ...
    console.log("[postversion] manifest.json -> " + version);
  } else {
    console.log("[postversion] manifest.json 已一致（" + version + "）");
  }
  ...
  console.log(
    "[postversion] cordis 包同步：" + cordisFiles.length + " 个（" + cordisChanged + " 个已写）-> " + version,
  );

  // 3) 真实同步门禁：仅当 version 阶段确实 bump 过（manifest/cordis 相对新主版本有实际
  //    写入）才提交。手动误跑/未 bump（版本未变）时派生全等 → 直接跳过 git（无论工作区
  //    是否有其它未提交改动——bump 的干净检查在 version 阶段，postversion 不替它越权提交杂项）
  if (!manifestChanged && cordisChanged === 0) {
    console.log("[postversion] 无派生版本同步（版本未变化或已一致），跳过 git commit/tag");
    return;
  }
  ...
  console.log("\n[postversion] ✅ v" + version + " 提交完成（package.json + manifest.json + " + cordisFiles.length + " 个 cordis 包对齐）。推送（tag 触发 CI 发布）：");
