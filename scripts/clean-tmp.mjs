...
//
// scripts/clean-tmp.mjs — 打包临时目录清理（package.json 的 postpack 钩子）
//
// 打包的中间原料（_tmp/pkg/ 铺平目录）与依赖暂存树（_tmp/pkg-root/）都可再生，
// 真正的产物只有 releases/ 下的 zip + sha256。本脚本把它们清掉。
//
// 为什么单独成脚本而不是内联在 pack.mjs 尾部：
//   · 声明式——package.json 里一眼可见"打包后要清临时目录"这条纪律；
//   · 可复用——CI 里 pack 步骤失败后也能单独调它把残留清掉；
//   · pack.mjs 只管出包，清理属生命周期职责。
//
// 用法：pnpm run postpack（由 pnpm run pack 自动触发）/ node scripts/clean-tmp.mjs
