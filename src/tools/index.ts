...
// 不串味（open 看不到 approvalId）。每个子命令是一个独立模块（open/reply/close/get/list/
// approve），本文件只做装配（name/description/parameters）与分发（execute）。
//
// 每个 action 模块导出同一组字段：
...
// close ≈ subagent_close（收工）；get / list / approve 是本项目特色（subagent 没有）。
...
import * as openAction from "./open.ts";
import * as replyAction from "./reply.ts";
import * as closeAction from "./close.ts";
import * as getAction from "./get.ts";
import * as listAction from "./list.ts";
import * as approveAction from "./approve.ts";

/** subcommand 注册表（顺序即 description 的列举顺序）。 */
const ACTIONS = [openAction, replyAction, closeAction, getAction, listAction, approveAction];
...
export const description =
  ...
  "get=回看某一轮最终结论；list=会话清单；approve=应答挂起审批（approvalId 必填）。" +
  "用法心智同 subagent：开、续、关；本项目另有 get/list/approve 三个特色动作。" +
