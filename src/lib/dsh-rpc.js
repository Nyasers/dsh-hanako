...
//
// src/lib/dsh-rpc.js — 经注入 fetch 的 DSH /api 网关 RPC 调用（App v2 步骤 4a）
//
// 背景：步骤 3 把「App 主进程 → 受管 runtime DSH web /api」的 loopback Unary RPC 封装
// 留在 lib/session-run.js 内部（ctx.network.fetch 门）。步骤 4a 需要同一条 RPC 面出现在
...
//     Node 全局 fetch（127.0.0.1 回环，dshana profile 无 BrowserAuth → 免鉴权，见
//     DESIGN「步骤 4a 架构决策」E）
