...
// 单例挂 g.bus：{ emit(channel, payload), on(channel, cb), status(), close() }。
...
/** 发 JSON 文本帧（未连接/未握手 no-op，返回是否送达） */
function sendFrame(frame) {
  if (!ws || ws.readyState !== 1 || !authed) return false;
  try {
...
/**
 * 挂单例 g.bus：跨模块（routes/webui.js 等经 globalThis 单例调用）访问总线。
 * bus.js 被 lifecycle.js 静态 import（内联进 dsh-run bundle），模块级状态即单例；
 * g.bus 暴露服务面（emit/on/status/close），与 g.closeProcess 等单例字段同纪律。
 */
getSingleton().bus = {
