...
//
// src/lib/session-serialize.js — 同 DSH session 提交串行化（App v2 步骤 3，决策 D）
//
// 背景（v1 tools/subtool/run.js 同款纪律）：DSH 的 queue 模式接受并发 prompt 并把后到
...
export function enterSessionTurn(sessionKey) {
  if (sessionTurnQueues.has(sessionKey)) {
    throw new Error("dsh_session 内部错误：新会话 " + sessionKey + " 已有排队提交");
  }
  let release = null;
  const gate = new Promise((resolve) => {
    release = resolve;
