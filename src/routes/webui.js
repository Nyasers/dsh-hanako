...
// ---- BFF 代理辅助：dsh cookie（经总线 authCookie 从 bridge 进程内换发，模块级缓存）----
// 代理 /webui/app/* 到官方 dsh web 时注入；浏览器侧不持有 dsh cookie（跨 site iframe
// 的 SameSite 墙），鉴权全部在宿主代理层消化（SPA 无感，即「抄 web-app 脱鉴权」）。
let dshProxyCookie = "";
let dshProxyCookieAt = 0;
async function getDshProxyCookie() {
  if (dshProxyCookie && Date.now() - dshProxyCookieAt < 6 * 3600 * 1000) {
    return dshProxyCookie;
  }
  try {
    const j = await callUnaryBus("authCookie", {});
    if (j && j.ok && typeof j.value === "string" && j.value) {
      dshProxyCookie = j.value;
      dshProxyCookieAt = Date.now();
      return dshProxyCookie;
    }
  } catch {
    /* 总线未就绪：沿用旧 cookie 或空 */
...
// 官方 SPA 路径改写（HTML/JS/SSE 帧通用）：绝对路径前缀 → 宿主代理前缀（同源化）
function rewriteDshUrl(text, prefix) {
  return String(text)
...
function buildShell({
  ...
  // iframe src 由壳页 JS 统一设置（attach）：透传宿主注入的 pluginSurfaceSession
  // query 到 BFF 代理（/webui/app/）——带凭据加载官方 SPA，宿主路由层放行。
...
export default function registerWebuiRoutes(app, ctx) {
  ...
  // 注入脚本：资源基址改写（SPA 全部资源由 @dsh-hanako/web-app 子插件（dsh 3080）
  // 提供——fetch/XHR/EventSource/WebSocket 从宿主域改写/重定向到 3080 子插件，
  // 跨源 CORS 已放开）+ DSHana 诊断横幅（web host 状态轮询宿主 health 端点）
  function buildInjections(port) {
    return `<script>
(function () {
  var BASE = "http://127.0.0.1:${port}";
  function rewrite(u) {
    var s = String(u);
    if (s.indexOf(BASE) === 0) return s;
    if (s.indexOf("/webui/") === 0) return BASE + s;
    if (s.indexOf("webui/") === 0) return BASE + "/" + s;
    return s;
  }
  if (window.fetch) {
    ...
    window.fetch = function (input, init) {
      var i = input;
      if (typeof input === "string") i = rewrite(input);
      else if (input && typeof input === "object" && input.url) {
        try { i = new Request(rewrite(input.url), input); } catch (e) {}
      }
  ...
  if (window.XMLHttpRequest) {
    ...
    X.prototype.open = function (m, u) {
      return origOpen.call(this, m, rewrite(String(u)));
    };
  }
  if (window.EventSource) {
    var ES = window.EventSource;
    window.EventSource = function (url, cfg) {
      return new ES(rewrite(String(url)), cfg);
    };
  }
  if (window.WebSocket) {
    ...
    window.WebSocket = function (url, protocols) {
      return protocols === undefined
        ? new WS(rewrite(String(url)))
        : new WS(rewrite(String(url)), protocols);
    };
...
(function () {
  ...
  box.id = "dshana-diag";
  box.style.cssText = "position:fixed;top:0;left:0;right:0;z-index:2147483647;font:12px/1.6 system-ui;padding:6px 14px;display:none;";
  document.body.appendChild(box);
  function show(state, text, extra) {
    box.style.display = state ? "block" : "none";
    if (!state) return;
    box.style.background = ({ ok: "#1e3a2a", warn: "#3a2e1e", bad: "#3a1e1e" })[state] || "#222";
    box.style.color = "#eee";
    box.textContent = text;
    if (extra) {
      var b = document.createElement("a");
      b.href = "health";
      b.style.cssText = "margin-left:8px;color:#8cf";
      b.textContent = extra;
      box.appendChild(b);
    }
  }
  function poll() {
    // 诊断是宿主端点：绝对 URL（location.origin）+ 宿主 token（页面 URL 的）
    var tok = new URLSearchParams(location.search).get("token") || "";
    var u =
      location.origin +
      "/api/plugins/dsh-hanako/webui/health" +
      (tok ? "?token=" + encodeURIComponent(tok) : "");
    fetch(u).then(function (r) { return r.json(); }).then(function (d) {
      if (!d || !d.ok) { show("bad", "DSHana：web host 未就绪", "查看诊断"); return; }
      var p = d.diagnostics && d.diagnostics.process;
      if (p && p.alive && p.ready) show("ok", "DSHana：web host 运行中");
      else if (p && p.alive) show("warn", "DSHana：web host 启动中…");
      else show("bad", "DSHana：web host 未运行", "查看诊断");
    }).catch(function () { show("bad", "DSHana：诊断不可用", "查看诊断"); });
  }
  poll();
  ...
  // 从宿主路由请求上下文提取凭据（token + pluginSurfaceSession；Hono c.req.query
  // 兼容性兜底：raw.url 正则解析）。token 是宿主 API 凭据（assets base 即 ?token=），
  // PSS 是 surface session——子资源请求两者都带。
  function credsFromReq(c) {
    const get = (name) => {
      try {
      ...
      try {
        const u = (c.req.raw && c.req.raw.url) || c.req.url || "";
        const m = String(u).match(new RegExp("[?&]" + name + "=([^&]+)"));
    ...
    return { pss: get("pluginSurfaceSession"), token: get("token") };
  }
  ...
  // 代理子路径（fork SPA 的动态面，同源化到 /webui/ 下）：
  //   /webui/plugins/* → 3080/plugins/*（官方：加载器/client 插件/SSE）
  //   /webui/api/*     → 3080/api/*（官方 API）
  //   其余（/webui/assets/* 等）→ 3080/webui/*（@dsh-hanako/web-app 子插件 serve 的 fork dist）
  // 注册 Hono v3（*）与 v4（{path*}）两种通配变体，兼容宿主路由版本。
  const proxySub = async (c) => {
    const pathname = c.req.path || "/";
    const rel = pathname.replace(/^\/webui/, "") || "/";
    const targetRel =
      rel.startsWith("/plugins/") || rel.startsWith("/api/")
        ? rel
        : `/webui${rel}`;
    const method = c.req.method || "GET";
    try {
      const cookie = await getDshProxyCookie();
      const headers = { cookie: cookie || undefined };
      let body;
      if (method !== "GET" && method !== "HEAD") {
        body = await c.req.arrayBuffer().catch(() => undefined);
        headers["content-type"] = c.req.header("content-type") || "application/json";
      }
      const res = await fetch(`http://127.0.0.1:${port}${targetRel}`, {
        method,
        headers,
        body,
        redirect: "manual",
      ...
      // SSE 事件流（/plugins/events）：帧流转发 + 帧内 URL 改写 + graph url 带 token
      if (ct.includes("event-stream")) {
        const encoder = new TextEncoder();
        const { token: tokF } = credsFromReq(c);
        const rewriteFrame = (fr) => {
          let s = rewriteDshUrl(fr, "/webui");
          if (tokF) {
            // graph 帧里的模块 url：SPA 动态创建 script 加载（无法经劫持加凭据）
            s = s.replace(
              /("url":\s*")([^"]+)(")/g,
              (m, p1, u, p3) => {
                if (u.includes("token=")) return m;
                return (
                  p1 +
                  u +
                  (u.includes("?") ? "&" : "?") +
                  "token=" +
                  encodeURIComponent(tokF) +
                  p3
                );
              },
            );
            ...
            try {
              ...
              while (true) {
                ...
                for (const p of parts) {
                  if (!p) continue;
                  controller.enqueue(encoder.encode(rewriteFrame(p) + "\n\n"));
                }
              }
              if (buf) controller.enqueue(encoder.encode(rewriteFrame(buf) + "\n\n"));
            } catch {
              /* 上游断开：结束流 */
            } finally {
              try {
        ...
        return c.body(stream, res.status, {
          "content-type": ct,
      ...
      // HTML/JS：文本读取 + 路径改写（同源化）；其余（图片/字体/JSON/二进制）透传
      const isText = ct.includes("html") || ct.includes("javascript");
      if (isText) {
        const text = rewriteDshUrl(await res.text(), "/webui");
        return c.body(text, res.status, {
          "content-type": ct,
        });
      }
      const buf = Buffer.from(await res.arrayBuffer());
      return c.body(buf, res.status, {
        "content-type": ct,
      });
    } catch (e) {
      return c.json({ ok: false, error: `代理失败：${(e && e.message) || e}` }, 502);
    }
  };
  // proxySub（宿主代理）已退役：fork SPA 资源全部由 @dsh-hanako/web-app 子插件
  // 提供（iframe 同源直嵌），宿主不再代理任何 dsh 路径。
  // app.all("/webui/{path*}", proxySub);
  // app.all("/webui/*", proxySub);
