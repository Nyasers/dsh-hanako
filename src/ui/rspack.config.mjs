...
// 产物：dist/ui/app-shell.js（ESM，浏览器 `<script type="module" src="./app-shell.js">` 直接加载）。
//
// 打包纪律：
//   - 浏览器 SDK @hana/plugin-sdk 从 devDependencies 解析（file:vendor/hana-app-sdk/
//     hana-plugin-sdk-0.0.0.tgz 的浏览器构建 dist/browser.js），由 rspack 静态打进本文件。
...
export default {
  ...
  mode: "production",
  target: "web",
  entry: path.join(root, "src", "ui", "app-shell.js"),
  output: {
    path: path.join(DIST_DIR, "ui"),
    filename: "app-shell.js",
    module: true,
    clean: false, // 主 bundle 已 clean 整树；这里只写 ui/app-shell.js（静态页由 build.js copy）
  },
