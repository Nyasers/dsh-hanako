...
标签页（**单主卡** DSHana：主卡 route `/dshana/main.html` 承载自举三态壳，就绪后内嵌 DSH main 视图（选中态桥接收端 receive）；同一张卡的 `functionPanel.route = /dshana/sidebar.html` 是主窗口功能面板，就绪后内嵌 DSH sidebar 视图（发射端 emit））按 `boot-state` 渲染三态，动作按钮只有「启动 DSH」/「停止 DSH」，无安装/检测入口：
