...
(function () {
  ...
  var sessionId = (root && root.dataset.session) || pageParams.get("sessionId") || "";
  var rpcId = (root && root.dataset.rpc) || pageParams.get("rpcId") || "";
  var timeoutMs = (root && root.dataset.timeout) || pageParams.get("timeoutMs") || "";
  ...
  if (!opId && !sessionId) { renderFail("缺少任务 ID"); return; }
    ...
    if (typeof text === "string" && text.indexOf(PREFIX) === 0) {
      try {
        ...
        for (var i = 0; i < blocks.length; i++) {
          ...
          else if (b.type === "reasoning" && b.text) out += '<details class="dsh-reasoning"><summary>思考过程</summary>' + mdToHtml(b.text) + "</details>";
          else if (b.type === "tool-call" && b.name) out += '<div class="dsh-toolcall">' + esc(b.name) + "</div>";
        }
        return out;
      } catch (e) { /* 解析失败回退 markdown */ }
    }
    ...
    if (typeof output === "string" && output.indexOf(PREFIX) === 0) {
      try {
        ...
        if (Array.isArray(blocks)) {
          S.blocksSeq = blocks.filter(function (b) { return b && typeof b === "object"; });
          S.collected = blocks.filter(function (b) { return b && b.type === "text" && b.text; }).map(function (b) { return b.text; }).join("");
          return;
        }
      } catch (e) { /* 解析失败按纯文本 */ }
    }
  ...
  function mergeUsageLocal(u) {
    ...
    if (u.cacheReadTokens != null) S.usage.cacheReadTokens = (S.usage.cacheReadTokens || 0) + u.cacheReadTokens;
    if (u.reasoningTokens != null) S.usage.reasoningTokens = (S.usage.reasoningTokens || 0) + u.reasoningTokens;
  }
  ...
  function startStream() {
    ...
    es.addEventListener("baseline", function (e) {
      var snap = null;
      try { snap = JSON.parse(e.data); } catch (err) { /* 坏帧忽略 */ }
      if (snap) onBaseline(snap);
    });
    es.addEventListener("event", function (e) {
      var frame = null;
      try { frame = JSON.parse(e.data); } catch (err) { /* 坏帧忽略 */ }
  ...
  function closeStream() {
    if (es) { try { es.close(); } catch (e) { /* 已关 */ } es = null; }
  }
  ...
  function fallbackStatus() {
    apiFetch("/ops/status?" + opQuery(), { cache: "no-store" })
      ...
      .catch(function () { renderFail("任务记录不存在"); });
  }
  ...
  function onBaseline(snap) {
    if (!snap || !snap.opId) { renderFail("任务记录不存在"); closeStream(); return; }
    S.base = snap;
    ...
    if (snap.status === "ok" || snap.status === "error") {
      ...
      if (snap.summary && snap.summary.text) { S.finalText = snap.summary.text; S.lastMsgText = snap.summary.text; }
    ...
    if (frame.type === "session/event") {
      ...
      if (ev.type === "assistant/chunk") {
        ...
        if (msgText) {
          ...
          if (!S.sawChunk) { S.collected += msgText; S.outputLength += msgText.length; }
  ...
  function onTurnEnd(d) {
    ...
    if (kind === "completed") { S.status = "ok"; S.stopReason = "end_turn"; }
    else if (kind === "max-tokens") { S.status = "ok"; S.stopReason = "max_tokens"; }
    else {
      S.status = "error";
      S.stopReason = kind || "end_turn";
      var f = (reason && (reason.failure || reason.error)) || {};
      S.error = String(f.message || (reason && reason.message) || "dsh 任务失败（无错误详情）");
    }
    if (!S.finalText) S.finalText = S.lastMsgText;
    if (S.base && S.base.startedAt) S.durationMs = Date.now() - new Date(S.base.startedAt).getTime();
    finishLocal();
    closeStream();
  }
  ...
  function startTicker() {
    if (tickTimer) return;
    var tms = S.base && S.base.timeoutMs != null ? Number(S.base.timeoutMs) : (timeoutMs ? Number(timeoutMs) : null);
    if (tms && S.base && S.base.startedAt) timeoutDeadline = new Date(S.base.startedAt).getTime() + tms;
    tickTimer = setInterval(function () {
      ...
      if (durEl && S.base && S.base.startedAt) {
        durEl.textContent = fmtDuration(Date.now() - new Date(S.base.startedAt).getTime()) + " · 进行中";
      }
    }, 1000);
  }
  ...
  function buildDisplayOp() {
    ...
    var fullText = S.blocksSeq.length ? "dsh-blocks-v1::" + JSON.stringify(S.blocksSeq) : S.collected;
    ...
    return {
      op: {
        ...
        provider: b.provider || "",
        model: b.model || "",
        timeoutMs: b.timeoutMs != null ? b.timeoutMs : (timeoutMs ? Number(timeoutMs) : null),
        status: S.status,
        startedAt: b.startedAt || null,
        durationMs: running ? null : S.durationMs,
        stopReason: S.stopReason,
        error: S.error,
        summary: (!running && summaryText) ? { text: summaryText, summaryOf: "final-message" } : null,
        usage: S.usage,
  ...
  function render() {
    ...
    var durMs = running && op.startedAt ? (Date.now() - new Date(op.startedAt).getTime()) : op.durationMs;
    // 摘要文本：终态有 summary 用最终结论；否则（运行中）用输出尾部预览
    var summaryText = (op.summary && op.summary.text) || op.outputPreview || built.summaryText || "";
    ...
    html += '<span class="dsh-title" title="DSHana">DSHana' + (op.agentPreset ? " · " + esc(op.agentPreset) : "") + "</span>";
    html += '<span class="dsh-badge ' + badgeCls + '">' + badge + "</span>";
    html += '<span class="dsh-dur">' + fmtDuration(durMs) + (running ? " · 进行中" : "") + "</span>";
    html += "</div>";
    html += '<div class="dsh-detail">';
    html += '<div class="dsh-d-row"><span class="dsh-d-label">任务</span><span class="dsh-d-value dsh-task-expand' + (S.taskOpen ? " open" : "") + '" title="' + (S.taskOpen ? "点击收起" : "点击展开/收起") + '">' + esc(op.task || "—") + "</span></div>";
    ...
    var modelText = [op.provider, op.model, op.reasoningEffort].filter(Boolean).join(" / ");
    if (modelText) html += '<div class="dsh-d-row"><span class="dsh-d-label">模型</span><span class="dsh-d-value">' + esc(modelText) + "</span></div>";
    // 时间行：开始时间 / 超时预算（无超时或恢复态只显示开始时间；超时预算并入避免独立行）
    var timeText = esc(fmtTime(op.startedAt));
    if (op.timeoutMs != null) timeText += " / " + esc(fmtDuration(op.timeoutMs));
    html += '<div class="dsh-d-row"><span class="dsh-d-label">时间</span><span class="dsh-d-value">' + timeText + "</span></div>";
    // 词元账目：始终显示（无 usage 时显示「—」，保持详情区行数稳定，卡片高度不随词元有无波动）
    var usageText = fmtUsage(op.usage);
    html += '<div class="dsh-d-row"><span class="dsh-d-label">词元</span><span class="dsh-d-value">' + esc(usageText || "—") + "</span></div>";
    // 状态：置于末行（badge + 停止原因）
    ...
    if (op.error) {
      ...
      var btnLabel = !S.fullOpen ? (running ? "滚动摘要" : "最终结论") : "完整输出";
      ...
      var bodyHtml = S.fullOpen ? renderBody(fullOutputText) : mdToHtml(summaryText);
    ...
    if (toggle) toggle.addEventListener("click", function () {
      ...
      if (body) body.innerHTML = S.fullOpen ? renderBody(lastFullText) : mdToHtml(S.lastSummaryText);
      toggle.textContent = S.fullOpen ? "完整输出" : (running ? "滚动摘要" : "最终结论");
      reportSize();
    });

    // 任务值：默认单行截断，点击展开/收起全文（状态存 JS 变量，重建后保持）
    var taskExpand = root.querySelector(".dsh-task-expand");
    if (taskExpand) taskExpand.addEventListener("click", function () {
    ...
    if (bodyEl && S.fullOpen && (replaced || fullOutputText !== S.lastFullRendered)) {
    ...
    if (durEl && running && op.startedAt) {
      durEl.textContent = fmtDuration(Date.now() - new Date(op.startedAt).getTime()) + " · 进行中";
    }
  ...
  // 连接占位（基线到达前避免空白闪烁）
  root.innerHTML = '<div class="dsh"><div class="dsh-empty">正在连接任务状态…</div></div>';
  reportSize();
  startStream();
})();
