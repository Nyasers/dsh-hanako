...
//
// tests/session-card.test.mjs — dshana_session 的 create/send 返回值里的会话流卡字面量。
//
// 卡是新增字段：details.dsh 的形状必须原样（SKILL 与句柄契约都读它）。提交链用
// deps.submitDshTask 注入 fake，不触真 runtime、不碰宿主任务面。
import test from "node:test";
import assert from "node:assert/strict";
import { doExecute } from "../src/tools/session.ts";
...
test("create：返回值带 card.route（ui 静态树路径 + ?ts= 快照），details.dsh 形状不变", async () => {
  const cwd = "E:\\Hanako\\workspace";
  const loc = { action: "create", sessionId: SID, rpcId: "rpc-1", taskId: "task-1", cwd };
  const { out, fake } = await run("create", loc, { task: "干活", cwd, context: { callToken: "tok" } });
  ...
  assert.equal(fake.calls[0].action, "create");
...
test("send：同样带卡；没有 cwd 时 route 不带 cwd、dsh 的 cwd 仍是缺省态", async () => {
  ...
  assert.equal(fake.calls[0].action, "send");

  const card = out.details.card;
  assert.equal(card.pluginId, "dshana");
  assert.match(card.title, /续发消息/);
  const q = cardQuery(card);
  assert.equal(q.get("action"), "send");
  assert.equal(q.get("sid"), SID);
  assert.equal(q.get("cwd"), null, "send 没有 cwd 就不塞这一格");
});
