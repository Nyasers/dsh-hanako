...
- **服务端**（dsh 进程内）：`@dsh-hanako/bridge` cordis 插件经 `webServer.registerUpgrade({ path:"/api/dshana.bus", handler })` 注册 upgrade 路由——dsh-host-webserver 的 WebServer 类原生支持 upgrade 路由（`server.on("upgrade")` 按 pathname 分发，handler 拥有 socket 完整协商权，源码已核实），handler 内零依赖手写 RFC6455（`ws-lib.js`：Sec-WebSocket-Key/Accept、帧解析、分片重组、256KB 单帧上限、连接级错误处理与 socket 清理）。握手完成后要求**首帧 hello** `{ channel:"hello", payload:{ token } }`，token 与 config 注入的 `busToken` 比对，不匹配立即关闭（close 1008）；5s 未发 hello 关闭；单连接语义（宿主唯一客户端，新连接 hello 通过后旧连接关闭）
- **客户端**（宿主插件）：`src/lib/bus.js` 用 Node 22+ 全局 WebSocket（零依赖）连 `ws://127.0.0.1:<webPort>/api/dshana.bus`，open 后首帧发 hello；断线指数退避重连（1s→封顶 30s）、应用级心跳 `bus.ping`/`bus.pong`（30s/90s 判死）；单例 `g.bus`（emit/on/status/close）。web host 就绪点（ensureWebHost 唯一就绪点）生成 `busToken`（每次 spawn 随机 16 字节 hex，存 g.busToken）→ 渲染 patch 注入 `{{BUS_TOKEN}}` → 就绪后 `connectBus`；closeProcess/插件卸载 `closeBus`
- **协议**：JSON 文本帧 `{ channel, payload }`——`update.request`（设置页发起）/ `update.result`（宿主回投 `{ state, version?, error? }`）/ `bus.ping`/`bus.pong`（心跳）。更新链路：设置页 request-update → dshanaBus.emit("update.request") → 宿主 bus 收到 → `g.updateDsh`（复用现有能力层：停 host → npm i latest → 起 host → 写 update-result.json）→ 完成后总线回投 update.result；并发防护复用 `g.updating`（进行中重复请求不重复执行）
- **降级**：bus 连接失败不阻断 dsh 启动（信道降级：request-update 报「消息总线未连接」，check-version 走 dsh 侧直查不受影响）；更新结果以 `update-result.json` 为准（设置页 update-status 轮询读），update.result 回投尽力而为
...
- **dsh 启动 patch overlay**：dsh-run.js spawn `dsh --profile web --patch <...> --port <...>`，启动前渲染单一模板 `dsh-plugin/dsh-hanako.patch.yml.tpl` 为机器绝对路径写数据目录 `dsh-hanako.patch.generated.yml`（五段：session-query 注入已移除（dsh 默认 openAt: never 禁用全文搜索，agent 只读自己创建的会话，见 tools/dsh-session.js）——@dsh-hanako/logger 统一日志服务注册 + @dsh-hanako/theme 主题插件注册 + @dsh-hanako/provider 宿主 provider 跟随注册 + @dsh-hanako/settings 设置页 DSHana 设置分页注册 + @dsh-hanako/bridge 进程间消息总线服务端注册（v0.13.0 由 default-model 改名升级，config 注入 `dshPkgDir`/`dataDir`——「检查与更新 DSH」链路：v0.18.1 曾注入 `npmCliPath`/`electronNode` 供设置页 dsh 侧 spawn pnpm view 直查远端（npmCliPath = 运行时引导的 pnpm 入口，`tools/lib/pnpm.js` `ensurePnpm` 产物），v0.18.2 起检查改 **HTTP 直查 npm registry**（全局 fetch，官方源 + npmmirror 兜底），两字段与模板占位符 `{{NPM_CLI_PATH}}`/`{{ELECTRON_NODE}}` 已删除；`dataDir` 供 update-result 文件；v0.22.1 起 hostApi 注入（/child/post 反向信道）删除，更新请求改经 bridge 段总线）；bridge 段 config 注入 `busToken`（`{{BUS_TOKEN}}`，每次 spawn 生成存宿主单例 g.busToken，子插件首帧 hello 校验）；v0.9.5 起 provider 段**恒渲染**——无配置项，宿主数据目录直接探测（插件安装形态 `<宿主数据目录>/plugins/<pluginId>` 上溯定位 models.json / provider-catalog.json），settings 段同样恒挂载；各段 cordis 插件均**包名注册**（`name: @dsh-hanako/logger / @dsh-hanako/theme / @dsh-hanako/provider / @dsh-hanako/settings / @dsh-hanako/clipboard / @dsh-hanako/bridge`，非 file:// URL，因 dsh client 模块发现按 `require.resolve('<name>/package.json')` 解析，file:// 不可解析），dsh-run.js spawn 前在 `$DSH_HOME/profiles/node_modules` 幂等创建 junction 指向插件安装目录 `dsh-plugin/@dsh-hanako/<同名>`（v0.18.1 统一 `@dsh-hanako` scope；与 dsh 自维护的 junction farm 同机制——dsh 的 `healProfilesModuleFallback` 只管理自身依赖闭包，不碰外来 junction）；启动时顺带清理 `dsh-hana-*` 旧名遗留 junction（dsh-hana-default-model / dsh-hana-proxy 等，无条件收敛杜绝混装））；模板缺失/渲染失败不挂任何 patch 记 warn（内嵌插件降级不可用，dsh 启动不受影响）——patch 缺失时优雅降级不阻断启动
