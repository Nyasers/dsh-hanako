...
- **功能面板（functionPanel，v1.0.1-alpha.1 方案 A：embedUrl 退役 + sections 重建）**：manifest 声明 `functionPanel: { id: "dshana-panel", label: "DSHana" }`（不再有 embedUrl——PR #33 曾以 embedUrl iframe 嵌 dsh Web UI 到宿主侧栏，宿主 0.810.0 对静态 loopback URL 原样透传、无端口改写，webPort 配置变更即失效，方案 A 退役该字段），面板内容由壳页运行时推送（`hana.panel.set`，宿主内置原语渲染，面板宽度 180–400px 可拖）：**list 段**（id `dshana-sessions`，label「会话」；items = dsh 会话清单（GET /webui/sessions，读 session_projcache.json，dsh-home 唯一事实源），字段与宿主渲染器兼容——title（空白会话兜底「新会话」）+ selected（当前会话标记）+ activation {kind:"emit"}，点击经 onEvent 收 select 事件）、**status 段**（web host 状态运行中/未就绪/启动中/安装依赖中 + 端口，deltaTone 按状态 success/danger/warning）、**actions 段**（新建会话 / 打开设置（dsh UI 操作，仅 web host 就绪可用）+ 手动启动 / 安装依赖 / 检测依赖 / 复制日志路径（web host 操作），`activation: { kind: "emit" }` 回传壳页执行）、**meta 段**（日志会话路径只读展示，有值才推）。**交互链路（方案 A）**：宿主侧栏面板点击（list 会话项 / actions 新建·设置）→ 宿主投递面板事件 → 壳页 `hana.panel.onEvent` 回调 → 壳页脚本 postMessage 主区 dsh iframe（`{ type:"dshana.act", action:"open-session"|"new-session"|"open-settings", sessionId?, title?, dupIndex?, absIndex? }`）→ `@dsh-hanako/sidebar` 注入脚本（?dshana=main）在**隐藏但 mounted 的 sidebar DOM** 里定位对应元素模拟点击（会话行按「标题→dupIndex→absIndex」匹配，New Session / 设置入口按 aria-label/aria-haspopup 选择器）→ dsh 前端切换会话/新建/设置 → 主区落实；主区 iframe 另经 MutationObserver 观察 aria-selected 变化回传当前会话（`{ type:"dshana.current", title, dupIndex, absIndex }`，来源限定 frame.contentWindow）→ 面板 selected 标记跟随（服务端无权威 current，最近活跃近似 + 主区回传覆盖）。壳页内置最小 hana 桥（与 @hana/plugin-sdk 同协议 hana.plugin.ui v1，插件 bundle 不引入 SDK 包），浏览器侧 fetch 一律走 `hana.api.fetch`（自动带 `X-Hana-Plugin-Surface-Session` 头，替代旧 surfaceHeaders 手动拼接）。
...
| `GET /webui/sessions` | 会话清单（面板 list section 数据源；读 dsh 会话持久化缓存 session_projcache.json，dsh-home 唯一事实源；返回 { ok, sessions: [{ id, title, current, dupIndex, absIndex }] }，current 为最近活跃近似，主区回传 dshana.current 覆盖） |
...
126: - **DSH 检查/更新能力层**（v0.13.0；v0.18.1 设置页检查改 dsh 侧直查）：`tools/lib/` 提取后挂单例 `g.checkDshUpdate` / `g.updateDsh` / `g.installDeps` / `g.verifyDeps`（Agent 工具 dsh_install/dsh_update、webui 路由共用，单一事实源；设置页检查 v0.18.1 起由 dsh 侧 @dsh-hanako/settings 内嵌直查）——`checkDshUpdate`（lib/check.js）：本地版本（verifyDepsSmoke 缓存优先/直读 dsh-pkg package.json）+ 远端版本（**HTTP 直查 npm registry**——fetch `https://registry.npmjs.org/@deepseek-ai/dsh/latest` 的 JSON `version` 字段（pnpm view 语义等价；官方源失败重试 npmmirror，15s 超时；v0.18.2 起替代 spawn pnpm view，不再依赖 pnpm 引导））+ zero-dep semver 比较 → `{ localVersion, latestVersion, updateAvailable, error? }`，结果缓存 `g.check.result`（内存，不再写 check-result.json 桥接文件）；`updateDsh`（dsh-run.js，组合 lib 的 installDepsFromPlugin/verifyDepsSmoke）：停 web host → npm i latest → 起 web host → 读新版本 → 结果走内存态 `g.update`（{ status, result, error, time }，v0.24 退役 update-result.json）；`installDepsFromPlugin` / `verifyDepsSmoke`（lib/install.js）：依赖部署与运行级验证；并发防护 `g.check.status` / `g.update.status` / `g.deps.status`（进行中重复调用返回状态不重复执行）
