...
| `dsh_session(action, limit?, sessionId?)` | 统一会话查询 | list=清单与摘要（仅 agent 创建的会话——config.json sessions 注册表作用域，projcache，limit 默认 10）；get=凭 sessionId 直取内容（summary，注册表外无权读取）；search 模式已移除 | [dsh-session 技能](dsh-session) |
